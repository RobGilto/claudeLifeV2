...
### Calendar Sync:
\`\`\`
node scripts/fractal-planner-llm.cjs calendar-sync ${plan.date}
\`\`\`
...
*Plan created: ${new Date().toLocaleString('en-AU', { timeZone: 'Australia/Sydney' })} | Sydney, Australia*`;
    ...
    async calendarSync(dateStr) {
        const dateIndex = new DateIndex(dateStr ? new Date(dateStr) : new Date());
        const identifiers = dateIndex.getIdentifiers();
        const display = dateIndex.getMultiIndexDisplay();
        
        console.log(`\n📅 Calendar Sync: ${identifiers.day}`);
        ...
        if (!plan) {
            console.log(`\n❌ No daily plan found for ${identifiers.day}`);
            console.log(`\n💡 Create a plan first: node scripts/fractal-planner-llm.cjs plan-day ${identifiers.day}`);
            return { success: false, reason: 'No plan found' };
        }

        if (!plan.timeBlocks || plan.timeBlocks.length === 0) {
            console.log(`\n❌ No time blocks found in plan for ${identifiers.day}`);
            return { success: false, reason: 'No time blocks' };
        }

        console.log(`\n🔄 Syncing ${plan.timeBlocks.length} time blocks to Google Calendar...`);
        ...
        for (const block of plan.timeBlocks) {
            try {
                const startDateTime = `${identifiers.day}T${block.start}:00`;
                const endTime = this.calculateEndTime(block.start, block.duration);
                const endDateTime = `${identifiers.day}T${endTime}:00`;
                ...
                const eventData = {
                    ...
                    colorId: this.getBlockColorId(block.type)
                };
        ...
        // Output calendar sync instructions
        console.log(`\n📋 Calendar Events Ready for Sync:`);
        calendarEvents.forEach((event, i) => {
            console.log(`   ${i + 1}. ${event.blockTitle} (${event.eventData.start.dateTime.split('T')[1].substring(0,5)} - ${event.eventData.end.dateTime.split('T')[1].substring(0,5)})`);
        });
        
        if (failures.length > 0) {
            console.log(`\n⚠️  Failed to prepare ${failures.length} time blocks:`);
            failures.forEach(failure => {
                console.log(`   - ${failure.blockTitle}: ${failure.error}`);
            });
        }
        ...
        const syncDataPath = path.join(DATA_DIR, `calendar-sync-${identifiers.day}.json`);
        const syncData = {
            date: identifiers.day,
            planId: plan.id,
    ...
    getBlockColorId(blockType) {
        // Google Calendar color IDs for different block types
        const colorMap = {
            'deep-work': '9',     // Blue - for focused deep work
            'learning': '10',     // Green - for learning and development  
            'admin': '6',         // Orange - for administrative tasks
            'review': '11',       // Red - for reviews and reflection
            'break': '2'          // Sage - for breaks and transitions
        };
        return colorMap[blockType] || '1'; // Default lavender
    }
    ...
    try {
        switch (command) {
            ...
            case 'status':
                ...
                break;
            case 'calendar-sync':
                await planner.calendarSync(args[1]);
                break;
            default:
                console.log(`
