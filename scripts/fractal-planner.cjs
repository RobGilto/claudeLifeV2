...
Generated by Fractal Planning System`;
    ...
    generateExecutiveInstructions(block) {
        // Generate specific instructions based on block type and activity
        const instructions = [];
        
        if (block.type === 'deep-work') {
            instructions.push('• Focus on deep, uninterrupted work session');
            instructions.push('• Eliminate distractions and notifications');
            instructions.push('• Use pomodoro or time-boxing if helpful');
        } else if (block.type === 'learning') {
            instructions.push('• Engage in active learning with note-taking');
            instructions.push('• Focus on understanding over completion');
            instructions.push('• Document key insights and breakthroughs');
        } else if (block.type === 'admin') {
            instructions.push('• Handle administrative tasks and communications');
            instructions.push('• Process inbox and respond to urgent items');
            instructions.push('• Plan and organize upcoming work');
        } else if (block.type === 'buffer') {
            instructions.push('• Use for project work overflow if needed');
            instructions.push('• Handle any admin tasks or quick planning');
            instructions.push('• Strategic rest option if energy is low');
        }
        
        // Add activity-specific instructions
        instructions.push(`• ${block.activity}`);
        
        return `📋 Executive Instructions:\n${instructions.join('\n')}`;
    }
    
    getValuesForBlockType(blockType) {
        const valueMap = {
            'deep-work': 'Excellence, focus, professional mastery',
            'learning': 'Growth, curiosity, skill development', 
            'admin': 'Organization, communication, efficiency',
            'buffer': 'Flexibility, sustainability, stress management',
            'review': 'Reflection, improvement, strategic thinking'
        };
        return valueMap[blockType] || 'Professional growth, strategic development';
    }
    ...
    saveUuidMappings(date) {
        if (!this.uuidMappings || this.uuidMappings.length === 0) {
            console.log(`  📝 No UUID mappings to save for ${date}`);
            return;
        }
        
        const mappingsDir = path.join(PLANNING_DIR, 'uuid-mappings');
        ...
        const mappingsData = {
            date: date,
            generated: getSydneyDate().toISOString(),
            mappings: this.uuidMappings,
            totalBlocks: this.uuidMappings.length
        };
        
        const mappingsFile = path.join(mappingsDir, `uuid-mappings-${date}.json`);
        fs.writeFileSync(mappingsFile, JSON.stringify(mappingsData, null, 2));
        console.log(`  💾 UUID mappings saved: planning/uuid-mappings/uuid-mappings-${date}.json`);
    }
    
    async createTaskWarriorTasks(blocks, date, mcpCommands = []) {
        ...
        try {
            ...
            for (let i = 0; i < blocks.length; i++) {
                ...
                // Enhanced metadata with UUID cross-linking
                taskCmd += ` -- "Time block: ${block.type} | Alignment: ${block.alignment} | FRACTAL_UUID:${fractalUuid} | EVENT_UUID:${eventUuid} | LLM_TRACKING_ID:${trackingUuid}"`;
                
                try {
                    execSync(taskCmd, { stdio: 'pipe' });
                    console.log(`  ✅ Task created: ${taskDescription}`);
                    
                    // Store UUID mappings for calendar integration
                    if (!this.uuidMappings) this.uuidMappings = [];
                    this.uuidMappings.push({
                        blockIndex: i,
                        fractalUuid,
                        eventUuid,
                        trackingUuid,
            ...
            // Save task sync record and UUID mappings
            this.saveTaskSyncRecord(date, blocks.length, successCount);
            this.saveUuidMappings(date);
            
            if (successCount > 0) {
                console.log(`\n  📋 TaskWarrior Integration:`);
                console.log(`     • ${successCount} time block tasks created`);
                console.log(`     • UUID mappings saved for calendar cross-linking`);
