...
class FractalPlanner {
    ...
    async createGoogleCalendarEvents(blocks, date) {
        try {
            ...
            if (!credentials) {
                console.log(`  ⚠️ Google Calendar API credentials not found`);
                return false;
            }

            const auth = await this.authenticateGoogle(credentials);
            const calendar = google.calendar({ version: 'v3', auth });
            
            console.log(`  📅 Creating ${blocks.length} calendar events via Google API...`);
            ...
            for (const block of blocks) {
                ...
                const startDateTime = `${date}T${block.start}:00+11:00`; // Sydney timezone
                const endDateTime = `${date}T${this.addMinutes(block.start, block.duration)}:00+11:00`;
                ...
                try {
                    ...
                    console.log(`  ✅ Created: ${eventTitle} (${response.data.id})`);
                    successCount++;
                } catch (eventError) {
                    console.log(`  ❌ Failed to create: ${eventTitle} - ${eventError.message}`);
                }
            }
            
            console.log(`  📊 Calendar sync complete: ${successCount} created, ${duplicateCount} duplicates skipped`);
            ...
            console.log(`  ⚠️ Google Calendar API failed: ${error.message}`);
            PlanStorage.log(`Calendar API error: ${error.message}`);
            return false;
        }
    }
    
    loadGoogleCredentials() {
        const credentialsPath = path.join(__dirname, '..', 'client_secret_google_calendar.json');
    ...
    async authenticateGoogle(credentials) {
        ...
        const tokenPath = path.join(__dirname, '..', 'google_calendar_token.json');
        ...
        throw new Error('No Google Calendar token found. Please run authentication first.');
    }
