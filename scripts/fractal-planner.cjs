...
class FractalPlanner {
    ...
    async createGoogleCalendarEvents(blocks, date) {
        try {
            // Check if we're running in an environment with MCP access
            // Since this is a Node.js script, we'll save event data for Claude Code to process
            const calendarData = [];
            let successCount = 0;
            
            console.log(`  📅 Creating ${blocks.length} calendar events...`);
            
            for (const block of blocks) {
                const eventData = {
                    calendarId: 'primary',
                    summary: `${this.getBlockTypeIcon(block.type)} ${block.activity}`,
                    start: `${date}T${block.start}:00`,
                    end: `${date}T${this.addMinutes(block.start, block.duration)}:00`,
                    timeZone: 'Australia/Sydney',
            ...
            // Save calendar data for Claude Code MCP processing
            const calendarDir = path.join(PLANNING_DIR, 'calendar-sync');
            if (!fs.existsSync(calendarDir)) {
                fs.mkdirSync(calendarDir, { recursive: true });
            }
            
            const syncFile = path.join(calendarDir, `auto-sync-${date}-${Date.now()}.json`);
            const syncData = {
                date: date,
            ...
            console.log(`  ✅ Calendar event data prepared: ${successCount} events`);
            console.log(`  📁 Saved to: ${path.relative(process.cwd(), syncFile)}`);
            console.log(`  🔄 Events ready for MCP sync - run with Claude Code for automatic creation`);
            
            // Return true to indicate we've handled calendar creation
            return true;
            
        } catch (error) {
            console.log(`  ⚠️ Calendar auto-sync failed: ${error.message}`);
            PlanStorage.log(`Calendar sync error: ${error.message}`);
            return false;
        }
    }
